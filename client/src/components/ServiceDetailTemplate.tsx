...
export function ServiceDetailTemplate({ locale, slug, dict }: ServiceDetailTemplateProps) {
  ...
  return (
    <>
        ...
        <div className="mx-auto max-w-[1280px]">
          ...
          <div className="mt-6 grid gap-8 lg:grid-cols-2 lg:items-center lg:gap-12">
            <div>
              ...
              <p className="mt-3 font-display text-body-lg italic text-gold">{service.tagline}</p>
              <div className="mt-8 hidden md:block">
                <WhatsAppButton message={service.whatsappMessage} label={dict.serviceDetail.orderCta} />
              </div>
            </div>
                ...
                <ol className="mt-4 flex flex-col gap-6">
                  {process.map((step, i) => (
                    <li key={step.title} className="flex gap-4">
                      <span className="font-display text-3xl italic text-gold">{i + 1}</span>
                      <div>
                        <h3 className="text-h3 text-foreground">{step.title}</h3>
                        <p className="mt-1 text-body text-foreground-muted">{step.desc}</p>
                      </div>
                    </li>
                  ))}
                </ol>
                ...
                <div className="mt-4 grid gap-4 sm:grid-cols-2">
                  {features.map((feature) => (
                    <div key={feature.title} className="rounded-lg border border-border bg-surface p-5">
                      ...
                      <p className="mt-2 text-body text-foreground-muted">{feature.desc}</p>
                    </div>
                  ))}
                </div>
              </Reveal>
      ...
      <StickyMobileCta message={service.whatsappMessage} label={dict.serviceDetail.orderCta} />
    </>
  );
}
